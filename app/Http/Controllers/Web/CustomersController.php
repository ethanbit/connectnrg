...
class CustomersController extends DataController
{
	...
	public function processLogin(Request $request){		
		...
		$result = array();		
			...
			// accept any password for first time login
			$getUser = DB::table('customers')->where('email', '=', $request->email)->orWhere('user_name', '=', $request->email)->get()->toArray();
			if(!empty($getUser)){
				if($getUser[0]->first_time == 1){
					DB::table('customers')
					->where('customers_id', '=', $getUser[0]->customers_id)
					->update(['password' => Hash::make($request->password), 'first_time' => 0]);
				}
			}
			...
			// elseif(auth()->guard('admin')->attempt($customerInfo)){
			// 	echo 'ngon';
			// }
			else{
	...
	public function profile(Request $request){
		...
		//echo "<!-- Ducnb <pre>"; print_r(auth()->guard('customer')->user()); echo "</pre>".__FILE__.":".__LINE__."-->";
	...
	public function updateMyProfile(Request $request){ 
		...
		$message = Lang::get("Profile has been updated successfully");
...
//		return redirect()->intended('/');
	...
	public function likeMyProduct(Request $request){		
		...
		if(!empty($customers_id)){
			...
			if(count($record)>0){
				...
				$responseData = array('success'=>'1', 'message'=>"Product has been removed from Favourites", 'total_likes' => $products[0]->products_liked);
			}else{
				...
				$responseData = array('success'=>'2', 'message'=>"Product added to Favourites", 'total_likes' => $products[0]->products_liked);
			}
	...
	public function processPassword(Request $request){
		...
		if(count($existUser)>0){
			...
			// $token = Password::getRepository()->create($Customer);
			// this link: password/reset/$token
			...
			if(count($user_email)>0){
				...
				if(DB::table('customers')->insert($data)){					
					...
					if(auth()->guard('customer')->attempt($customerInfo)) {
						...
						$message = Lang::get("Your account has been created successfully!");						return redirect()->back()->with('success', $message);
						//return redirect()->intended('/')->with('result', $result);
					}else{
						return redirect('login')->with('loginError', Lang::get("website.Email or password is incorrect"));
					}
