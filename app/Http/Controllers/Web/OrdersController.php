...
class OrdersController extends DataController
{
	...
	public function checkout(Request $request){
		...
		if(count($result['cart'])==0){			
			...
			$shippingObj = new ShippingAddressController();

			// address book		
			$result['address_book'] = $shippingObj->getShippingAddress(''); 
			$result['countries_book'] = $shippingObj->countries();
			
			//tax rate
			$tax_rate_setting_data = DB::table('settings')->where('id',93)->get();
			$tax_rate_setting= $tax_rate_setting_data[0]->value;
			$result['tax_rate_setting'] = $tax_rate_setting;
	...
	public function checkout_shipping_address(Request $request){
		...
		$addressBook = array();
		$addressBook['customers_id']         = auth()->guard('customer')->user()->customers_id;
		$addressBook['entry_company']        = $shipping_data['company'];
		$addressBook['entry_firstname']      = $shipping_data['firstname'];
		$addressBook['entry_lastname']       = $shipping_data['lastname'];
		$addressBook['entry_street_address'] = $shipping_data['street'];
		$addressBook['entry_suburb']         = $shipping_data['suburb'];
		$addressBook['entry_postcode']       = $shipping_data['postcode'];
		$addressBook['entry_city']           = '';
		$addressBook['entry_state']          = $shipping_data['zone_id'];
		$addressBook['entry_country_id']     = $shipping_data['countries_id'];
		$addressBook['entry_zone_id']        = $shipping_data['zone_id'];
		$addressBook['entry_phone']        = $shipping_data['delivery_phone'];

		if($shipping_data['address_book_id'] != ''){
			DB::table('address_book')->where('address_book_id', $shipping_data['address_book_id'])->update($addressBook);
		}else{
	...
	public function place_order(Request $request){
		...
		$order_no_part1=substr(str_shuffle($str_result1), 0, 3); 
		$order_no_part2=substr(str_shuffle($str_result2), 0, 2); 
		$purchase_order_no =$order_no_part1.$order_no_part2;
		...
		$delivery_suburb            			=   session('shipping_address')->suburb;
		...
		$delivery_date            				=   @session('shipping_address')->delivery_date;
		$delivery_note            				=   session('shipping_address')->delivery_note;
		...
		$billing_suburb	            			=   session('billing_address')->billing_suburb;
		...
		//$delivery_date            				=   session('billing_address')->billing_delivery_date;
		
		if(!empty(session('billing_company')->billing_compan)){
		...
		$total_tax_new= $shipping_cost*$tax_rate_setting/100;
...
//						'updatedAt'=>$result->transaction->updatedAt->date,
		...
		//echo 'payment_method:'.$payment_method.'-- payment_status:'.$payment_status; exit;
		//check if order is verified
		if($payment_status=='success'){
			
			$orders_id = DB::table('orders')->insertGetId(
				[	 				
					 'tax_rate' => $tax_rate_setting,		// from the admin setting in the admin panel
					 'order_no' => $order_no,		
					...
					// 'orders_status' => $orders_status,
				 	...
				 	// 'total_tax'		 =>		$total_tax,
				 	 'total_tax'		 =>		$order_price*($tax_rate_setting/100),
					 'ordered_source' 	 => 	'1',
			...
			$ordersData['order_no']    				=	$order_no;
			
			//notification/email
			//echo '<pre>'; print_r($ordersData); echo '</pre>'.__FILE__.':'.__LINE__;exit;
			...
			if(Auth::guard('api')->check()){
				...
				return redirect('order-thankyou/'.$orders_id)->with('success', Lang::get("website.Payment has been processed successfully"));
			}
	...
	public function getOrders(){
		...
		foreach($orders as $key => $orders_data){
			...
			$orders_status_history = DB::table('orders_status_history')
				...
				->where('orders_id', '=', $orders_data->orders_id)->orderby('orders_status_history.orders_status_history_id', 'DESC')->limit(1)->get()->toArray();
	...
	public function orderThankyou(Request $request){
		...
		if(count($orders)>0){
			...
			$result['orders'] = $orders;
			return view("view-order_thankyou", $title)->with('result', $result); 
		}else{
			return redirect('orders');
		}
	}
	...
	public function shipping_methods(){
		...
		foreach($cart as $products_data){	
			if(!empty($products_data->weight) or is_numeric($products_data->weight)){
		...
		foreach($shippings as $shipping_methods){
			...
			if($shipping_methods->methods_type_link == 'upsShipping' and $shipping_methods->status == '1'){
				...
				foreach($carts as $cart){
					if(!empty($cart->weight) or is_numeric($cart->weight)){
						$weight += $cart->weight*$cart->customers_basket_quantity;
					}
				}
	...
	public function updateThePrice(Request $request){
	 $customer_basket_id=$_POST['customer_basket_id'];
	 $new_qty=$_POST['new_qty'];
	 $basketData = array();
		//$basketData['customers_id']         = auth()->guard('customer')->user()->customers_id;
		$basketData['customers_basket_quantity']      = $new_qty;
		

		if($customer_basket_id != ''){
			DB::table('customers_basket')->where('customers_basket_id', $customer_basket_id)->update($basketData);
	...
	public function exportReport(){
		...
		header( 'Content-Disposition: attachment;filename=Report_'.date('d-m-Y_H-i-s').'.csv');
