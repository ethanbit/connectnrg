...
class ProductsController extends DataController
{
    ...
    public function shopcategories(){
		...
		$pageTitle = Lang::get('website.Shop');

		//echo "<pre>"; print_r($result); echo "</pre>".__FILE__.":".__LINE__;exit;

    	return view('categories', $title)->with('result', $result);
    }
	...
	public function shop(Request $request){
		...
		if(!empty($request->category) and $request->category!='all'){
			...
			if($category[0]->parent_id==0){
				...
				$sub_category_name = '';
				$category_slug = '';
				$main_sub_category_data = DB::table('categories')->leftJoin('categories_description','categories_description.categories_id','=','categories.categories_id')->where('categories.parent_id',$categories_id)->where('language_id',Session::get('language_id'))->get();
		...
		$result['sub_category_name'] = $sub_category_name;
		  ...
		  foreach($main_sub_category_data as $key=>$subCat)
		 {
		 
		$main_sub_category_data_products = DB::table('products_to_categories')->where('products_to_categories.categories_id',$subCat->categories_id)->get();
		if(!empty($main_sub_category_data_products))
		{
		//$result['sub_category_data_count'][$key]['cat_id'] = $subCat->categories_id;
		//$result['sub_category_data_count'][$key]['cat_count']=count($main_sub_category_data_products);
		$result['sub_category_data_count'][]=count($main_sub_category_data_products);
		}
		else
		{
		//$result['sub_category_data_count'][$key]['cat_id'] = $subCat->categories_id;
		//$result['sub_category_data_count'][$key]['cat_count']=0;
		$result['sub_category_data_count'][]=0;
		}
		//$result['sub_category_data'] = count($main_sub_category_data_products);
		 }
		  
		
		$result['sub_category_data']=$main_sub_category_data;
		
		/*  echo "<pre>";
		  print_r($result['sub_category_data']);
		  print_r($result['sub_category_data_count']);
		   die("stop");  */

		if(!empty($request->manufacturer) and $request->manufacturer!='all'){
			$manufacturers = DB::table('manufacturers')
			 				->LeftJoin('manufacturers_info', 'manufacturers.manufacturers_id', '=', 'manufacturers_info.manufacturers_id')
			 				->where('manufacturers_slug', $request->manufacturer)
			 				->where('languages_id',Session::get('language_id'))
			 				->first();
			
			$manufacturer_id = $manufacturers->manufacturers_id;
			$manufacturer_slug = $manufacturers->manufacturers_slug;
		...
		$result['manufacturer_slug'] = $manufacturer_slug;
		...
		$data = array('page_number'=>$page_number, 'type'=>$type, 'limit'=>$limit, 'categories_id'=>$categories_id, 'search'=>$search, 'filters'=>$filters, 'limit'=>$limit, 'min_price'=>$min_price, 'max_price'=>$max_price, 'manufacturer_id' => $manufacturer_id );	
		...
		$data = array('limit'=>$limit, 'categories_id'=>$categories_id, 'manufacturer_id' => $manufacturer_id );
	...
	public function filters($data){
		...
		$price = DB::table('products_to_categories')
						...
						->join('manufacturers', 'products.manufacturers_id', '=', 'manufacturers.manufacturers_id');
		if(isset($categories_id) and !empty($categories_id)){
			$price->where('products_to_categories.categories_id','=', $categories_id);
		}

		if(isset($manufacturer_id) and !empty($manufacturer_id)){
			$price->where('products.manufacturers_id','=', $manufacturer_id);
		}
		...
		if(isset($manufacturer_id) and !empty($manufacturer_id)){
			$product->where('products.manufacturers_id','=', $manufacturer_id);
		}
