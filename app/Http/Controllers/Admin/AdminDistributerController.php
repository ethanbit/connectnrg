...
class AdminDistributerController extends Controller

{
	...
	public function adddistributers(Request $request){	


		$title = array('pageTitle' => Lang::get("labels.AddManufacturer"));

		return view("admin.adddistributers",$title);


	}

	
	public function addnewdistributers(Request $request){
		$finduserbyemail = DB::table('distributers')->select('id')->where('distributor_email', $_POST['distributor_email'])->first();
		if(count($finduserbyemail) <= 0){
			...
			return redirect()->back()->withErrors("Disributor Added Successfully");
	   }
	   else{
	...
	public function getdistributor($language_id){

		$distributers = DB::table('distributers')->select('*')->get();
		return($distributers);
	}
