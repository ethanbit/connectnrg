...
{	
	...
	public function customers(Request $request){
		...
		$QueryObj = DB::table('customers')
			...
			//->paginate(50);
		$customers = $QueryObj->paginate(50);
		$totalCustomers = $QueryObj->count();
		$result = array();
		$index = 0;
		foreach($customers as $customers_data){
			...
			$groups = DB::table('customers_group')
			->join('groups', 'customers_group.group_id', '=', 'groups.id')
			->select('groups.*')
			->where('customer_id','=',$customers_data->customers_id)->get();
			$result[$index]->groups = $groups;
		...
		$customerData['groups'] = $groups;
		$customerData['totalCustomers'] = $totalCustomers;

		return view("admin.customers",$title)->with(['customers' => $customerData, 's' => '']);
		}
	}
	...
	public function searchcustomers(Request $request){
		$s = trim($request->customersearch);
		$group = trim($request->group);
		if($s != '' or $group != ''){
			// echo DB::table('customers')
			// ->where('email', 'LIKE', '%'.$s.'%')
			// ->orWhere('customers_firstname', 'LIKE', '%'.$s.'%')
			// ->orWhere('customers_telephone', 'LIKE', '%'.$s.'%')
			// ->toSQL();
			...
			if(session('customers_view')==0){
				...
				if($s != ''){
					$QueryObj->where('customers.email', 'LIKE', '%'.$s.'%');
					$QueryObj->orWhere('customers.customers_firstname', 'LIKE', '%'.$s.'%');
					$QueryObj->orWhere('customers.customers_telephone', 'LIKE', '%'.$s.'%');
				}

				if($group != ''){
					$QueryObj->join('customers_group', 'customers_group.customer_id', '=', 'customers.customers_id');
					$QueryObj->where('customers_group.group_id', '=', $group);
				}

				$QueryObj->orderBy('customers.customers_id','ASC'); 
				...
				//echo "<pre>"; print_r($customers); echo "</pre>".__FILE__.":".__LINE__;
				...
				return view("admin.customers",$title)->with(['customers' => $customerData, 's' => $s]);
			}
		}else{
	...
	public function addaddress(Request $request){
		...
		$defaultAdd = DB::table('customers')
			->select('customers_default_address_id')
			->where('customers_id', '=', $customers_id)
			->get()
			->toArray();	
		//echo "<pre>"; print_r($defaultAdd); echo "</pre>".__FILE__.":".__LINE__;exit;
		...
		$customerData['defaultAdd'] = $defaultAdd[0]->customers_default_address_id;	
	...
	//print_r('test');	die;
		if(session('customers_create')==0){
			print Lang::get("labels.You do not have to access this route");			//print_r('if');			die;
		}else{	 
				//print_r('else');
	...
	public function editAddress(Request $request){
		...
		$countries = DB::table('countries')->get();	
		$zones = DB::table('zones')->where('zone_country_id','=', 13)->get(); 
	...
	public function editcustomers(Request $request){
		...
		$user_group = DB::table('groups')->select('groups.*')
					->join('customers_group', 'customers_group.group_id', '=', 'groups.id')
					->join('customers', 'customers.customers_id', '=', 'customers_group.customer_id')
					->where('customers.customers_id','=', $customers_id)->get()->toArray();
		...
		$customerData['errorMessage'] = $errorMessage;
		$customerData['customers'] = $customers;
		$customerData['user_group'] = $user_group;
		$customerData['groups'] = $groups;
	...
	public function updatecustomers(Request $request){
		...
		DB::table('customers_group')->where('customer_id', '=', $customers_id)->delete();
		for($i = 0; $i < count($user_group); $i++){
			DB::table('customers_group')->insert(['group_id' => $user_group[$i], 'customer_id' => $customers_id]);
		}
		...
		if($request->old_email_address!=$request->email){
			...
			if(count($existEmail)>0){
				...
				$customerData = array();
				$message = array();
				$errorMessage = array('Email address already exist.');
				...
				$customerData['user_group'] = $user_group;
				$customerData['groups'] = $groups;
				//echo "<pre>"; print_r($customerData); echo "</pre>".__FILE__.":".__LINE__;exit;
	...
	public function deletecustomers(Request $request){
		if(session('customers_delete')==0){
			...
			return redirect('admin/customers')->withErrors([Lang::get("labels.DeleteCustomerMessage")]);
		}
	}
