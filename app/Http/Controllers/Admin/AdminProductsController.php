...
class AdminProductsController extends Controller
{
	...
	public function products(Request $request){
		...
		$data = DB::table('products')
			...
			if(isset($_REQUEST['product']) and !empty($_REQUEST['product'])){
				...
				$data->orWhere('products_model', 'like', '%' . $_REQUEST['product'] . '%');
			}
	...
	public function addproduct(Request $request){
		...
		$myVar = new AdminDistributerController();
		$result['distributors'] = $myVar->getdistributor($language_id);
		//tax class
	...
	public function addnewproduct(Request $request){
		...
		if($request->hasFile('products_image') and in_array($request->products_image->extension(), $extensions)){
			...
			$uploadImage = 'resources/assets/images/site_images/1586580478.logo_mobile.png';
		}	
		...
		$products_id = DB::table('products')->insertGetId([
					...
					'products_weight'		 =>   @$request->products_weight?$request->products_weight:'',
					'products_status'		 =>   $request->products_status,
					'products_tax_class_id'  =>   $request->tax_class_id,
					'products_weight_unit'	 =>	  @$request->products_weight_unit?$request->products_weight_unit:'',
					'low_limit'				 =>   0,
					'products_type'			 =>	  $request->products_type,
					'is_feature'			 =>	  $request->is_feature,
					'products_min_order'	 =>	  @$request->products_min_order?$request->products_min_order:'',
		...
		// }
		//return redirect('admin/editproduct/'.$products_id);
	...
	public function updateproduct(Request $request){
		...
		if($request->slug != ''){
			if($request->old_slug != $request->slug ){
				...
				//do{
					$slug = $myVar->slugify($request->slug);
					$slug_count = DB::table('products')->where('products_slug',$slug)->where('products_id','!=',$products_id)->count();
			...
			$slug = $myVar->slugify($request->products_name_1);
		}
	...
	public function importproducts(Request $request){
		...
		$title = array('pageTitle' => "Import Products");
		$language_id      =   '1';
		$result=array();
		return view("admin.importproduct", $title)->with('result', $result);
	...
	public function importproductsdata(Request $request)
	 {
      ...
      if(in_array(strtolower($extension),$valid_extension)){
          ...
          $location = 'resources/assets/csvs';

          // Upload file
          $file->move($location,$filename);

          // Import CSV to Database
          $filepath = $_SERVER['DOCUMENT_ROOT']."/".$location."/".$filename;
		...
		if($importData[2]==0 || $importData[2]=="")
		{
		$importData[2]=$importData[1];
		}
		else
		{
		$importData[2]=$importData[2];
		}
		if($importData[7]!=0 || !empty($importData[7]))
		{
		$importData[7]=trim(str_replace("$","",$importData[7]));
        ...
        foreach($importData_arr as $importData){
        	...
        	$manufacturers = DB::table('manufacturers')->select('manufacturers.manufacturers_id as m_id')->where('manufacturers.manufacturers_name', $importData[3]);
			if($manufacturers)
			{
			$man_data = $manufacturers->get()[0];
			}
			else
			{
			$man_data = '';
            ...
            $insertProductsData = array(
               "products_model"=>$importData[0],
               //"products_image"=>'resources/assets/images/site_images/1586580478.logo_mobile.png',
               "products_image"=>'resources/assets/images/product_images/'.$importData[0].'.jpg',
               "products_price"=>$importData[10],
               "products_date_added"=>date('Y-m-d h:i:s'),
               "products_status"=>1,
               "manufacturers_id"=>$man_data->m_id,
               "products_slug"=>strtolower(str_replace(" ","-",$importData[1])),
               "unit_of_measure"=>$importData[7],
               "uploaded_using_csv"=>1
			   );
			
			//insert the sku, price  and unit in products
			$product_id = DB::table('products')->insertGetId($insertProductsData);
			if($product_id)
			{
				 //$insertProductsDescData = array("products_name"=>@$importData[1],"products_description"=>@$importData[2]);
				 $insertProductsDescData = array("products_name"=>$importData[1],"products_description"=>$importData[2]);
				 //$product_description = DB::table('products_description')->insertGetId($insertProductsDescData);
				 $product_description = DB::table('products_description')->insert($insertProductsDescData);
			}
			/*----------------Parent category----------------*/
			$cat_name=ucfirst($importData[4]);

			$categories = DB::table('categories_description')
						->select('categories_description.categories_id as cat_id')
						->where('categories_description.language_id', $language_id)
						->where('categories_description.categories_name', @$cat_name);
			$cat_data= $categories->get()[0];
			if($cat_data){
				$insertCatData = array("products_id"=>$product_id,"categories_id"=>$cat_data->cat_id);
			} else {
				$insertCatData = array( "products_id"=>$product_id,"categories_id"=>'8');
			}
			   
			$product_cat = DB::table('products_to_categories')->insert($insertCatData);	 


			/*----------------Sub category----------------*/

			if(!empty($importData[5])){
			$sub_cat_name=ucfirst($importData[5]);

				$sub_catgory = DB::table('categories_description')
						->select('categories_description.categories_id as cat_id')
						->where('categories_description.language_id', $language_id)
						->where('categories_description.categories_name', @$sub_cat_name);
				$s_cat_data= @$sub_catgory->get()[0];
				if($s_cat_data)
				{
					$insertsub_Data = array("products_id"=>$product_id,"categories_id"=>$s_cat_data->cat_id);
					$product_sub_cat = DB::table('products_to_categories')->insert($insertsub_Data);
				} 
			}
	...
	return redirect('admin/products/');
	 ...
	 public function importproductsdataOLD(Request $request)
	 {
      ...
      if(in_array(strtolower($extension),$valid_extension)){
		...
		/* if($importData[2]==0 || $importData[2]=="")
		{
		$importData[2]=$importData[2]?$importData[2]:$importData[1];
		}
		else
		{
		$importData[2]=$importData[2];
		...
		/* echo "<pre>";
		print_r($importData_arr_new);
		 die("stop");  */
        foreach($importData_arr_new as $importData){
			$language_id     =   $language_id;	
        	$manufacturers = DB::table('manufacturers')->select('manufacturers.manufacturers_id as m_id')->where('manufacturers.manufacturers_name', @$importData[3]);
			if($manufacturers)
			{
			$man_data = $manufacturers->get()[0];
			$man_id= $man_data->m_id;
			}
			else
			{
			$man_data = "";
			...
			$product_model=$importData[0];
			$image='resources/assets/images/product_images/'.@$importData[0].'.jpg';
			$products_price=$importData[7];
			$products_date_added=date('Y-m-d h:i:s');
			$products_status=1;
			$manufacturers_id=$man_id;
			$products_slug=strtolower(str_replace(" ","-",@$importData[1]));
			$unit_of_measure=@$importData[6];
			$uploaded_using_csv=1;
			...
			$insertProductsData = array(
               ...
               "uploaded_using_csv"=>$uploaded_using_csv
			   );
			...
			$categories = DB::table('categories_description')
						->select('categories_description.categories_id as cat_id')
						->where('categories_description.language_id', $language_id)
						->where('categories_description.categories_name', @$importData[4]);
			$cat_data= $categories->get()[0];
			if($cat_data){
				$insertCatData = array("products_id"=>$product_id,"categories_id"=>$cat_data->cat_id);
			} else {
			...
			if(!empty($importData[5])){
				...
				$sub_catgory = DB::table('categories_description')
						->select('categories_description.categories_id as cat_id')
						->where('categories_description.language_id', $language_id)
						->where('categories_description.categories_name', @$importData[5]);
				$s_cat_data= @$sub_catgory->get()[0];
				if($s_cat_data)
				{
					$insertsub_Data = array("products_id"=>$product_id,"categories_id"=>$s_cat_data->cat_id);
