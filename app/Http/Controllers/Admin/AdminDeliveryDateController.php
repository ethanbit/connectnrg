...
class AdminDeliveryDateController extends Controller
...
{		
	...
	public function deliverydatesetting(Request $request){
			...
			return view("admin.delivery",$title)->with('result', $result);
	...
	public function addnewdelivery(Request $request){
		...
		$dates = $request->delivery_dates;
		//$inserted = DB::table('delivery_dates')->insert(['delivery_dates'=>$datenew]);
		DB::table('delivery_dates')->update(['delivery_dates'=>$dates]);
		return redirect()->back()->withErrors(['Updated dates']);
