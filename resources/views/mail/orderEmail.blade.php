...
<div style="background: #fff; padding: 9px 10px;">
	<div class="top-mail" style="">
		...
		<h2 style="color: #d3742a; font-size: 20px; font-weight: 300; margin-top: 50px">Thank you for your order</h2>
	</div>
</div>
<div style="background: #fff; padding:10px;">
	<p style="clear:both;">Your order has been received and is now being processed. Your order details are shown below for your reference:</p>
  ...
  <table class="table table-striped" style="border-collapse: collapse; width: 100%;background-color: transparent;margin: 15px 0 15px;">
    ...
    <tbody style="text-transform: capitalize;font-size: 12px;">
	 ...
	 ?>
      <tr if($key%2==0){ style="background-color: #fff;" } >
        ...
        <td align="left" style="border: 1px solid #f4f4f4; padding:12px 8px;font-size:14px;">${{  number_format($products->products_price,2) }}</td>
        <td align="left" style="border: 1px solid #f4f4f4; padding:12px 8px;font-size:14px;">${{  number_format($eachProductTotal,2) }}</td>
      </tr>
     <?php } ?>
      </tr>
	 <tr>
        ...
        <td align="left" style="border: 1px solid #f4f4f4; padding:12px 8px;font-size:14px;">${{ number_format($allProductsTotal,2) }}</td>
	</tr>
	<?php 
	...
	<tr>
        ...
        <td align="left" style="border: 1px solid #f4f4f4; padding:12px 8px;font-size:14px;">${{ number_format($GST,2) }}</td>
	</tr>	
	<tr>
		<td align="left" style="border: 1px solid #f4f4f4; padding:12px 8px; font-size:14px;"></td>
        <td align="left" style="border: 1px solid #f4f4f4; padding:12px 8px; font-size:14px;"></td>
        ...
        <td align="left" style="border: 1px solid #f4f4f4; padding:12px 8px;font-size:14px;">${{ number_format($TOTAL,2) }}</td>
     </tr>
  ...
  <div style="display: block; width: 100%;padding: 0 0 20px; clear: both;">
    ...
    <div style="display: inline-block; width:48%"> <strong style="   font-size:18px;   color: #557da1;">{{ trans('labels.ShippingInfo') }}:</strong>
      ...
      <p>Order Comments: {{ $ordersData['orders_data'][0]->delivery_note }}</p>
    </div>
  </div>
  <!-- /.row --> 
  <!-- /.content --> 
