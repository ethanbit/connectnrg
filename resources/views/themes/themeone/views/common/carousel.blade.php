...
foreach($result['slides'] as $key=>$slides_data) {
	$slideArr[$slides_data->type][] = $slides_data;
}
foreach($slideArr as $key=>$v) {
?>
<div id="myCarousel_{{ $key }}" class="carousel slide {{ $key }}" data-ride="carousel">

	<ol class="carousel-indicators">
		@foreach($v as $key=>$slides_data)
	...
	<div class="carousel-inner">
		@foreach($v as $key=>$slides_data)
			<div class="carousel-item  @if($key==0) active @endif">			
				<img width="100%" class="first-slide"  src="{{asset('').$slides_data->image}}" width="100%" alt="{{ $slides_data->title}}">
