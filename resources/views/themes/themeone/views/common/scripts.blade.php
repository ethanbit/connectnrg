...
jQuery( document ).ready( function () {
	...
	var d = new Date();
    var checkCurrentTime = d.getHours();
    var showCurrentDay = 1;
    if(checkCurrentTime > cutOffTime){
    	//showCurrentDay = 2; 
		showCurrentDay = 9;
    }	else	
	{		
    ...
    console.log('showCurrentDay', showCurrentDay, 'checkCurrentTime', checkCurrentTime, 'cutOffTime', cutOffTime);
	jQuery('#delivery_date').datepicker({ 
		dateFormat: 'dd/mm/yy',  
		minDate: showCurrentDay,
		beforeShowDay: checkAvailability
	});
	jQuery('span.dhupdatecart').click(function(){
		var getPID = jQuery(this).data('product_id');
		var getQty = jQuery(this).parent().find('input.qty').val();
		var getcartID = jQuery(this).data('cart_id');
		jQuery.ajax({
			url: '/updatesinglecart',
			type: "POST",
			data: 'products_id='+getPID+'&cart_id='+getcartID+'&quantity='+getQty,		
			success: function (res) {
				jQuery('.head-cart-content').html(res);	
	...
	jQuery(document).on('click', '#cash_on_delivery_button', function(e){	
		...
		var checkErrors = jQuery('#dh_form_shipping .has-error').length;
		var isEmpty    = false;
		var firstname     = jQuery('#dh_form_shipping #firstname').val();
		var lastname      = jQuery('#dh_form_shipping #lastname').val();
		var company       = jQuery('#dh_form_shipping #company').val();
		var street        = jQuery('#dh_form_shipping #street').val();
		var suburb        = jQuery('#dh_form_shipping #suburb').val();
		var delivery_date = jQuery('#dh_form_shipping #delivery_date').val();
		var purchase_order_no = jQuery('#dh_form_shipping #purchase_order_no').val();
		var entry_zone_id = jQuery('#dh_form_shipping #entry_zone_id').val();
		if(firstname == '' || lastname == '' || company == '' || street == '' || suburb == '' || delivery_date == '' || purchase_order_no == '' || entry_zone_id == ''){
			isEmpty = true;
		}

		if(checkErrors == 0 && !isEmpty){
		jQuery('#loader').removeClass('loader').addClass('loader_show'); 
			var formData = jQuery('#dh_form_shipping').serialize();
			jQuery.ajax({
				type: 'POST',
				url: jQuery('#dh_form_shipping').attr('action'),
				data: formData,
				success: function(msg){
					if(!jQuery('#same_billing_address').prop('checked') == true){
						jQuery.ajax({
							type: 'POST',
							url: jQuery('#dh_form_billing').attr('action'),
							data: jQuery('#dh_form_billing').serialize(),
							success: function(msg){
								var checkPaymentMethodStatus = paymentMethods();
								if(checkPaymentMethodStatus){
									var formData = jQuery('#shipping_mehtods_form').serialize();
									jQuery.ajax({
										type: 'POST',
										url: jQuery('#shipping_mehtods_form').attr('action'),
										data: formData,
										success: function(msg){
											//setTimeout(function(){
												jQuery("#update_cart_form").submit();
			...
			jQuery('#dh_form_shipping input, #dh_form_shipping select').blur();
	...
	jQuery(document).on('click', '#same_billing_address', function(e){		
		if(jQuery(this).prop('checked') == true){
			jQuery('#d_address_book').val('');
			jQuery('#dh_form_billing #billing_wrapper').addClass('d-none');

			jQuery("#billing_firstname").val(jQuery("#firstname").val());
			jQuery("#billing_lastname").val(jQuery("#lastname").val());
			jQuery("#billing_company").val(jQuery("#company").val());
			jQuery("#billing_street").val(jQuery("#street").val());
			jQuery("#billing_suburb").val(jQuery("#suburb").val());
			...
			jQuery('#dh_form_billing #billing_wrapper').removeClass('d-none');
		}
	});	

	// select exist address for billing
	jQuery('#d_address_book').change(function(){
		var firstname = jQuery('option:selected', this).data('firstname');
		var lastname  = jQuery('option:selected', this).data('lastname');
		var street    = jQuery('option:selected', this).data('street');
		var suburb    = jQuery('option:selected', this).data('suburb');
		var postcode  = jQuery('option:selected', this).data('postcode');
		var city      = jQuery('option:selected', this).data('city');
		var zoneid    = jQuery('option:selected', this).data('zoneid');
		var company   = jQuery('option:selected', this).data('company');
		var billing_phone     = jQuery('option:selected', this).data('phone');
		 console.log("the p is:"+billing_phone);
		jQuery("#billing_firstname").val(firstname);
		jQuery("#billing_lastname").val(lastname);
		jQuery("#billing_company").val(company);
		jQuery("#billing_street").val(street);
		jQuery("#billing_city").val(city);
		jQuery("#billing_zip").val(postcode);			
		jQuery("#billing_countries_id").val(13);
		jQuery("#billing_zone_id").val(zoneid);	
		jQuery("#billing_suburb").val(suburb);
		jQuery("#billing_phone").val(billing_phone);		
	})

	// select exist address for shipping
	jQuery('#d_shipping_address_book').change(function(){
		var address_book_id = jQuery(this).val();
		var firstname       = jQuery('option:selected', this).data('firstname');
		var lastname        = jQuery('option:selected', this).data('lastname');
		var street          = jQuery('option:selected', this).data('street');
		var suburb          = jQuery('option:selected', this).data('suburb');
		...
		var zoneid          = jQuery('option:selected', this).data('zoneid');
		var company         = jQuery('option:selected', this).data('company');
		var entry_phone     = jQuery('option:selected', this).data('phone');

		jQuery("#address_book_id").val(address_book_id);
		...
		//jQuery("#billing_countries_id").val(13);
		jQuery("#entry_zone_id").val(zoneid);	
		jQuery("#suburb").val(suburb);	
		jQuery("#entry_phone").val(entry_phone);	
	})
		...
		jQuery.ajax({			
			...
			success: function (res) {			
				...
				if(obj.success==0){
					...
					jQuery(selector).removeClass('fa-star-o');
					jQuery(selector).addClass('fa-star');
					jQuery(selector).children('span').html(obj.total_likes);
					jQuery('#wishlist-count').html(parseInt(user_count)+ parseInt(1));	
					jQuery(selector).children('.badge').html(obj.total_likes);
				}else if(obj.success==1){
					jQuery(selector).removeClass('fa-star');
					jQuery(selector).addClass('fa-star-o');
...
jQuery(document).on('click', '.cart', function(e){	
	...
	var products_id = jQuery(this).attr('products_id');
	var qty = jQuery(this).parents('.product').find('input.qty').val();
	var message ;
	jQuery.ajax({
		url: '{{ URL::to("/addToCart")}}',
		type: "POST",
		...
		success: function (res) {
			if(res.trim() == "already added"){							
				...
				setTimeout(function(){
					...
					jQuery(parent).html("@lang('website.Add to Cart')");
				}, 2000);
			}
			message = "@lang('website.Product is added')";			
			notification(message);
...
jQuery(document).ready(function(){
	...
	if(jQuery('#entry_zone_id').length){
		var checkState = jQuery('#entry_zone_id').val();
		if(checkState == ''){
			getZones();
		}
	}
	...
	jQuery('.reorder').click(function(){
		var productsID = jQuery(this).attr('products_id');
		var productIDArr = productsID.split(',');
		if(productIDArr.length > 0){
			...
			for(var i=0; i < productIDArr.length; i++){
				jQuery.ajax({
					...
					type: "POST",
					async: false,
					data: '&products_id='+productIDArr[i]+'&quantity=1',		
					success: function (res) {
						addedToCart = true;
					},
				});
	...
	var owl = jQuery('.homepage_specialproduct_slider .owl-carousel');
	owl.owlCarousel({
		loop:true,
		margin:10,
	...
	jQuery('.productslidernav .next').click(function(){
		owl.trigger('next.owl.carousel');
	})
	jQuery('.productslidernav .prev').click(function(){
		owl.trigger('prev.owl.carousel');
	})

	var owl2 = jQuery('.homepage_categories_slider .owl-carousel');
	owl2.owlCarousel({
		loop:true,
		margin:10,
		responsiveClass:true,
	...
	jQuery('.categoriesslidernav .next').click(function(){
		owl2.trigger('next.owl.carousel');
	})
	jQuery('.categoriesslidernav .prev').click(function(){
...
<script>
 //the qty update on minus
jQuery(document).on("click",".qtyminuscart",function(){
  
  var theQt=jQuery(this).next('.updateQty').val();
   if(theQt !="" && theQt >1)
   {
  var customer_basket_id=jQuery(this).next('.updateQty').attr('customer_basket_id');
  var thePrice=jQuery(this).next('.updateQty').attr('price_of_one');
  console.log("the qt:"+theQt);
   console.log("the thePrice:"+thePrice);
   var theLastPrice=jQuery(".theSubTotal").text();
    var theNewPrice=parseFloat(theLastPrice)- parseFloat(thePrice);
    var subTotal =theNewPrice.toFixed(2);
    var the_tax_rate =jQuery(".theGST").attr('the_tax_rate');
	var GST=subTotal*(parseFloat(the_tax_rate)/100);
	var theGST=GST.toFixed(2);
	var total =parseFloat(subTotal)+ parseFloat(theGST);
	var finalTotal =total.toFixed(2);
   console.log("the qt:"+theQt);
   console.log("the thePrice:"+thePrice);
   console.log("the theLastPrice:"+theLastPrice);
   console.log("the subTotal:"+subTotal);
   console.log("the the_tax_rate:"+the_tax_rate);
   console.log("the GST:"+theGST);
   console.log("the Total:"+finalTotal);
   console.log("the customer_basket_id:"+customer_basket_id);
   jQuery(".theSubTotal").text(subTotal);
   jQuery(".theGST").text(theGST);
   jQuery(".theTotal").text(finalTotal);  
   
   jQuery.ajax({
			url: '{{ URL::to("/updatetheprice")}}',
			type: "POST",
			//data: '&customer_basket_id='+customer_basket_id,
			data: '&customer_basket_id='+customer_basket_id+'&new_qty='+theQt,			
			success: function (res) {},
		});
   }
 });
...
//the qty update on plus
jQuery(document).on("click",".qtypluscart",function(){
  
  var theQt=jQuery(this).prev('.updateQty').val();
   if(theQt !="" && theQt >1)
   {
   var customer_basket_id=jQuery(this).prev('.updateQty').attr('customer_basket_id');
  var thePrice=jQuery(this).prev('.updateQty').attr('price_of_one');
  console.log("the qt:"+theQt);
   console.log("the thePrice:"+thePrice);
   var theLastPrice=jQuery(".theSubTotal").text();
    var theNewPrice=parseFloat(theLastPrice)+ parseFloat(thePrice);
